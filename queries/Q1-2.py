...
def convert_address_to_lat_long(address):
    ...
    return {"latitude": 60.83, "longitude": -10.89}  # Example coordinates
...
fresh_product = db.products.aggregate([
    { "$match": { "mainCategory": "Fresh" } },
    { "$sample": { "size": 1 } }
]).next()
...
nearest_partner = db.partners.find({
    "currentLocation": {
        "$nearSphere": {
            "$geometry": {
                "type": "Point",
                "coordinates": [customer_location["latitude"], customer_location["longitude"]]
            }
        }
    },
    "status": "active"
